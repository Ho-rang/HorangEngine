...
bool Handle_C_SIGNUP2(Horang::PacketSessionRef& session, Protocol::C_SIGNUP2& pkt);
...
public:
	static void Init()
	{
		...
		GPacketHandler[PKT_C_SIGNUP2] = [](Horang::PacketSessionRef& session, BYTE* buffer, int32 len) { return HandlePacket<Protocol::C_SIGNUP2>(Handle_C_SIGNUP2, session, buffer, len); };
	}
...
private:
	...
	static Horang::SendBufferRef MakeSendBuffer(T& pkt, uint16 pktId)
	{
		...
		Horang::PacketHeader* header = reinterpret_cast<PacketHeader*>(sendBuffer->Buffer());
